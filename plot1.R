...
global_active_power = sapply(subset_data[,3], function(x) as.numeric(as.character(x)))

#Open device, write histogram, default settings for compatibility
png(filename='plot1.png', width=480, height=480)
hist(global_active_power, main="Global Active Power", col="red", axes='true', border='black', xlab='Global Active Power (kilowatts)',
     ylab='Frequency')
dev.off()
